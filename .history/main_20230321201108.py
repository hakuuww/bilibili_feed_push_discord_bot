...
async def create_channel(guild, category_name, channel_name):
    category = awai discord.utils.get(guild.categories, name=category_name)
    if category is None:
        category = guild.create_category(category_name)
    channel = await discord.utils.get(category.channels, name=channel_name)
    if channel is None:
