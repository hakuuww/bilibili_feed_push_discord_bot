...
async def create_channel(guild, category_id, channel_name):
    
    category = discord.utils.get(client.guild.categories, id='put the id here without apostrophes')
    if not category:
  ...
  if message.content.startswith('hello'):
    ...
    for dynamic_obj in dynamic_page_info:
        ...
        if id_already_exists is not None:
            ...
            category_id = 1087885108343738408   
             
             
            channel = await create_channel(guild, category_id, channel_name)
